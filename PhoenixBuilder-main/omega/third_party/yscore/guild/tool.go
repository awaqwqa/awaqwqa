...
func (b *Guild) RefreshTheScore() {
	...
	for k, v := range list {
		...
		if ok {
			//如果在公会内 且分数不一样则同步
			if playerScore, isok := v[b.DictScore["公会同步积分计分板"]]; isok && playerScore != b.GuildData[guildname].GuildRankings {
				...
				pterm.Info.Printfln("已将%v的公会计分板分数同步为:%v", k, b.GuildData[guildname].GuildRankings)
			} else {
				b.CmdSender(fmt.Sprintf("scoreboard players set %v %v %v", k, b.DictScore["公会同步积分计分板"], strconv.Itoa(b.GuildData[guildname].GuildRankings)))
				pterm.Info.Printfln("已将%v的公会计分板分数同步为:%v", k, b.GuildData[guildname].GuildRankings)
			}
