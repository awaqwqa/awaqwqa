...
func (b *Guild) Activate() {
	...
	//初始化各种计分板
	for k, v := range b.DictScore {
		b.Frame.GetGameControl().SendCmdAndInvokeOnResponse(v, func(output *packet.CommandOutput) {
			if output.SuccessCount > 0 {
				pterm.Info.Printfln("%v计分板检测到没有 已自动创立计分板%v", k, v)
			} else {
				pterm.Info.Printfln("%v计分板已经存在计分板名字叫", k)
			}
	...
	go func() {
		...
		for _, v := range b.GuildData {
			if v.GuildRankings == 0 {
				...
				pterm.Info.Printfln("当前公会积分分数已更新为:%v", v.GuildRankings)
			}
		}
